CLASS zcl_ey_aaa_amdp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get_customer_by_id IMPORTING
                                       VALUE(i_bp_id) TYPE zey_aaa_dte_id
                                     EXPORTING
    ...
    CLASS-METHODS get_product_mrp IMPORTING
                                    VALUE(i_tax) TYPE i
                                  EXPORTING
                                    VALUE(otab)  TYPE zey_aaa_tt_product_mrp.

       CLASS-METHODS get_total_sales for table FUNCTION ZEY_Aaa_TF.
  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.
...
CLASS ZCL_EY_AAA_AMDP IMPLEMENTATION.
 ...
 OPTIONS READ-ONLY.
  ...
  METHOD get_customer_by_id BY DATABASE PROCEDURE FOR HDB LANGUAGE SQLSCRIPT
                           options read-only
                           USING zey_aaa_bpa .
    select concat( 'M/s ', company_name ) as company_name into e_res from zey_aaa_bpa where bp_id = :i_bp_id;
  ENDMETHOD.


  METHOD get_product_mrp BY DATABASE PROCEDURE FOR HDB LANGUAGE SQLSCRIPT
                            OPTIONS READ-ONLY
                            USING zey_aaa_product.
*   declare variables
...
*   get all the products in a implicit table (like a internal table in abap)
    lt_prod = select * from zey_aaa_product;
...
*   fill the otab for result (like in abap we fill another internal table with data)
 ...
 METHOD get_total_sales by DATABASE FUNCTION FOR HDB LANGUAGE SQLSCRIPT
                       OPTIONS READ-ONLY
                       USING zey_aaa_bpa zey_aaa_so_hdr zey_aaa_so_item.
   ...
   return select
    ...
    from zey_aaa_bpa as bpa
   INNER join zey_aaa_so_hdr as sls
   on bpa.bp_id = sls.buyer
   inner join zey_aaa_so_item as item
 ...
 ENDMETHOD.
  ...
  METHOD if_oo_adt_classrun~main.
    ""Ctrl+Space for code completion , Shift+Enter to generate code ,ctrl+7 for comment
*    zcl_ey_aaa_amdp=>add_numbers(
*      EXPORTING
*        a      = 30
*        b      = 40
*     IMPORTING
...
*   ).
    ...
    ""Copy the bp_id from output and paste here in line 58

*    zcl_ey_aaa_amdp=>get_customer_by_id(
*      EXPORTING
*        i_bp_id = '227C11BB05441EEFB9891AC0CF171E47'
*      IMPORTING
*        e_res   = DATA(lv_result)
*    ).
    zcl_ey_aaa_amdp=>get_product_mrp(
      EXPORTING
        i_tax =  18
      IMPORTING
        otab  = DATA(result)
